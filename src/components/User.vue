<template>
    <v-layout align-start>
        <v-flex>
            <v-data-table :headers="headers" :items="users" sort-by="name" class :search="search">
                <template v-slot:top>
              ...
              <v-card-text>
                <v-container>
                  <v-row>
                    ...
                    <v-flex xs12 sm6 md6>
                      <v-text-field v-model="name" label="Nombre y Apellido"></v-text-field>
                    </v-flex>
          ...
          <v-dialog v-model="showStatusModal" max-width="300">
            <v-card>
              <v-card-text class="text-center pt-5 body-1">
                Esta accion
                <span class="text-decoration-underline">{{statusItemText}}</span> el usuario
...
export default {
    data: () => ({
        ...
        headers: [
            ...
            { text: "Rol", value: "rol", sortable: true, align: "start" },
            { text: "Tipo de Documento", value: "typeDni", sortable: true, align: "start" },
            { text: "Número de Documento", value: "numDni", sortable: true, align: "start" },
    ...
    methods: {
        ...
        getData() {
            let self = this;
            axios
                .post("/user/list", {customerId: this.currentUser.user.customerId}, this.configHeader)
                .then(res => {
                    self.users = res.data;
                    self.updateUserList();
                })
        ...
        updateUserList() {
            ...
            let filteredUsers = [];
            this.users.map(user => {
                if(user._id != self.currentUser.user._id){
        ...
        validateSubmit() {
            ...
            if (this.password.length < 1 || this.password.length > 64) {
                this.validateMessage.push(
                    "La contraseña debe tener entre 1-64 caracteres."
                );
            }
            ...
            if (this.editedIndex > -1) {
                ...
                axios
                    .put("user/update", {
                        ...
                        customer: this.currentUser.user.customerId
                    }, this.configHeader)
                    .then(res => {
                        self.reset();
                        self.close();
